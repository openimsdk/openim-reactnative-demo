...
export const ResetPasswordClient = async (params: {
  phoneNumber: any;
  password: string;
  verifyCode: any;
}) => {
...
export const getBusinessUserInfo = async (
  userIDs: string[],
  isSelfInfo = false,
): Promise<{users: BusinessUserInfo[]} | {error: string}> => {
  try {
    ...
    const response = await axios.post<{
      data:
        ...
        | {error: string}
        | PromiseLike<{users: BusinessUserInfo[]} | {error: string}>;
      users: BusinessUserInfo[];
    }>(
    ...
    if (error instanceof Error) {
      return {
        error: error.message || 'Unknown error occurred in getBusinessUserInfo',
      };
    }
    return {error: 'Unknown error occurred in getBusinessUserInfo'};
  }
};

export const searchBusinessUserInfo = async (
  keyword: string,
): Promise<{total: number; users: BusinessUserInfo[]} | {error: string}> => {
  try {
    const token = await AsyncStorage.getItem('chatToken');
    if (!token) throw new Error('Authentication token not found');
    ...
    const response = await axios.post<{
      data:
        | {total: number; users: BusinessUserInfo[]}
        | {error: string}
        | PromiseLike<
            {total: number; users: BusinessUserInfo[]} | {error: string}
          >;
      total: number;
      users: BusinessUserInfo[];
    }>(
    ...
    return {
      error:
        error.message || 'Unknown error occurred in searchBusinessUserInfo',
    };
  }
};
